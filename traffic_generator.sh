...
from scapy.all import sendp, IP, UDP, Raw, Ether, RandString

def generate_udp_packets(num_packets, payload_size, destination="169.254.1.4", port=12345):
    ...
    if payload_size:
         ...
         for i in range(num_packets):
            packet = Ether(dst="1c:34:da:54:9a:a4", src="b8:83:03:6f:63:51") / IP(dst=destination) / UDP(dport=port, sport=port) / Raw(load=s)
            sendp(packet, iface="cx5if1", verbose=False)
    else:
        for i in range(num_packets):
            packet = Ether(dst="1c:34:da:54:9a:a4", src="b8:83:03:6f:63:51") / IP(dst=destination) / UDP(dport=port, sport=port) / Raw(load="Test Packet "+str(i)+"\n")
            sendp(packet, iface="cx5if1", verbose=False)
...
if __name__ == "__main__":
    ...
    generate_udp_packets(args.num_packets,payload_size=args.payload_size)
